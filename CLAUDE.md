...
- **作品集**：`design`（設計，含 RC Car PDF、ESP32 天氣站、Arduino RTC 時鐘、WiFi 紅綠燈、自製 IR 遙控器）、`code`（程式，含工廠薪資系統）、`photo`（攝影集）
...
- `index.html` 裡的作品集內容（工廠薪資系統、ESP32 天氣站、Arduino RTC 時鐘、WiFi 紅綠燈、自製 IR 遙控器）是用 JavaScript 字串硬編碼在頁面內（`designProjects` 物件 + `codeProjects` 物件），不是獨立的 `.js` 資料檔。新增設計作品需同時加卡片 HTML 與 `designProjects` 的對應條目。
