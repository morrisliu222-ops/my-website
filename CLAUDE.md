...
```js
const posts = [
  { title: "標題", date: "YYYY-MM-DD", content: "內文或 HTML 字串" },
  // 最新文章排在最前面
];
```

`content` 欄位支援純文字或 HTML（直接用 `innerHTML` 渲染）。可在內文嵌入圖片：

```html
<img src="https://github.com/user-attachments/assets/<uuid>" alt="說明" />
```

**圖片來源**：透過 GitHub Issues 上傳（手機 App 或網頁），圖片存在 GitHub 的 AWS S3（user-attachments），不佔 git repo 空間，單張上限 10 MB。圖片 URL 不保證永久有效。

**圖片渲染行為**（`renderBlog()` 內處理）：
- 圖片正常 → 等比縮放顯示（CSS `.post-content img`：`max-width: 100%; height: auto`）
- 圖片載入失敗 → 自動替換為 `⚠️ 圖片載入失敗` 提示（`.img-error` 樣式）
...
- 技術筆記的 `content` 欄位是 HTML 字串，直接用 `innerHTML` 渲染。生活雜記的 `content` 也支援 HTML（可嵌圖片），同樣用 `innerHTML` 渲染。
